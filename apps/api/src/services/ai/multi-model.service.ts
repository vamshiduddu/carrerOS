...
async function callOpenAiCompatible(input: ChatInput) {
...
async function callGemini(input: ChatInput) {
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${input.model}:generateContent?key=${apiKey}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
async function callAzureOpenAI(input: ChatInput) {
  const apiKey = input.apiKey ?? getProviderKey('azure');
  if (!apiKey) {
    throw new Error('Missing API key for provider: azure');
  }

  const baseUrl = input.baseUrl ?? openAiLikeDefaults['azure'];
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data?.error?.message ?? `Azure OpenAI request failed (${response.status})`);
  }
...
async function callAnthropic(input: ChatInput) {
...
export async function chatWithModel(input: ChatInput) {
  ...
  if (input.provider === 'azure') {
    const result = await callAzureOpenAI(input);
    return { ...result, provider: input.provider, model: input.model };
  }

  const result = await callOpenAiCompatible(input);
  return { ...result, provider: input.provider, model: input.model };
}
